...
export function Booking() {
  ...
  const validateStep = (step: number) => {
    ...
    if (step === 1) {
      ...
      else if (!/\S+@\S+\.\S+/.test(formData.email)) newErrors.email = t('auth.error.email');
    ...
    if (step === 3 && !formData.date) {
      newErrors.date = t('auth.error.required');
    }
    setErrors(newErrors);
  ...
  const isStepValid = () => {
    if (currentStep === 1) return formData.firstName && formData.lastName && /\S+@\S+\.\S+/.test(formData.email) && formData.phone.length >= 7;
    if (currentStep === 2) return !!formData.treatment;
    if (currentStep === 3) return !!formData.date;
    return true;
  };
        ...
        <AnimatePresence mode="wait">
          {currentStep === 5 ? (
            ...
            >
              ...
              <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-secondary mb-4 italic">{t('booking.success.title')}</h2>
              ...
              <div className="p-6 sm:p-12">
                <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8">
                  {currentStep === 1 && (
                    <div className="space-y-8">
                      <div className="text-center">
                        <h2 className="text-2xl sm:text-3xl font-bold text-secondary mb-2 italic">{t('booking.step1')}</h2>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                        <div className="space-y-4">
                          ...
                          <input 
                            ...
                            className={`w-full px-4 sm:px-6 py-3 sm:py-4 rounded-2xl border bg-muted/10 ${errors.firstName ? 'border-red-500/50' : 'border-border'} focus:border-primary focus:ring-4 focus:ring-primary/5 outline-none transition-all text-base sm:text-lg`}
                          />
                        ...
                        <div className="space-y-4">
                          ...
                          <input 
                            ...
                            className={`w-full px-4 sm:px-6 py-3 sm:py-4 rounded-2xl border bg-muted/10 ${errors.lastName ? 'border-red-500/50' : 'border-border'} focus:border-primary focus:ring-4 focus:ring-primary/5 outline-none transition-all text-base sm:text-lg`}
                          />
                        ...
                        <div className="space-y-4 sm:col-span-2">
                          ...
                          <input 
                            ...
                            className={`w-full px-4 sm:px-6 py-3 sm:py-4 rounded-2xl border bg-muted/10 ${errors.email ? 'border-red-500/50' : 'border-border'} focus:border-primary focus:ring-4 focus:ring-primary/5 outline-none transition-all text-base sm:text-lg`}
                          />
                  ...
                  {currentStep === 2 && (
                    <div className="space-y-8">
                      <div className="text-center">
                        <h2 className="text-2xl sm:text-3xl font-bold text-secondary mb-2 italic">{t('booking.step2')}</h2>
                  ...
                  {currentStep === 3 && (
                    <div className="space-y-8">
                      <div className="text-center">
                        <h2 className="text-2xl sm:text-3xl font-bold text-secondary mb-2 italic">{t('booking.step3')}</h2>
                        <p className="text-muted-foreground">{t('booking.step3.desc')}</p>
                      </div>
                      <div className="space-y-6">
                        <div className="relative">
                          <CalendarIcon className="absolute left-4 sm:left-6 top-1/2 -translate-y-1/2 text-primary w-5 h-5 pointer-events-none rtl:left-auto rtl:right-4 rtl:sm:right-6" />
                          <input 
                            type="date" 
                            id="booking_date"
                            required 
                            ...
                            className="w-full pl-12 sm:pl-14 pr-4 sm:pr-6 rtl:pr-12 rtl:sm:pr-14 rtl:pl-4 rtl:sm:pl-6 py-3 sm:py-4 rounded-2xl border bg-muted/10 border-border focus:border-primary outline-none transition-all font-bold text-secondary text-base sm:text-lg"
                          />
                        </div>
                        <div className="space-y-4">
                          <label htmlFor="booking_message" className="text-sm font-bold uppercase tracking-widest text-muted-foreground pl-1 rtl:pl-0 rtl:pr-1">
                          ...
                          <textarea 
                            ...
                            className="w-full px-4 sm:px-6 py-3 sm:py-4 rounded-2xl border bg-muted/10 border-border focus:border-primary outline-none transition-all text-base sm:text-lg resize-none"
                          />
                        </div>
                      </div>
                    </div>
                  ...
                  {currentStep === 4 && (
                    <div className="space-y-8">
                      <div className="text-center">
                        <h2 className="text-2xl sm:text-3xl font-bold text-secondary mb-2 italic">{t('booking.step4')}</h2>
                      ...
                      <div className="bg-muted/30 rounded-2xl sm:rounded-[2rem] p-5 sm:p-8 space-y-6 border border-border/50">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 sm:gap-x-12 gap-y-6 rtl:text-right">
                          <div className="sm:col-span-2">
                            ...
                            <p className="text-lg sm:text-xl font-bold text-secondary">{formData.firstName} {formData.lastName}</p>
                          </div>
                          <div>
                          ...
                          <div>
                            ...
                            <p className="text-base sm:text-lg font-bold text-secondary break-all">{formData.email}</p>
                          </div>
                          <div>
                            <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-1">{t('booking.phone')}</p>
                            <p className="text-base sm:text-lg font-bold text-secondary" dir="ltr">{formData.phone}</p>
                          </div>
                        </div>
                      </div>
