...
export const WhatsAppWidget: React.FC = () => {
  ...
  // Configuration from dashboard state
  const CLINIC_NUMBER = '905413392569';
  const rawPhone = state?.whatsapp?.phoneNumber?.replace(/[^0-9]/g, '') || '';
  const phoneNumber = rawPhone.length >= 10 ? rawPhone : CLINIC_NUMBER;
  
  const defaultMessage = "Hello Gravity Clinic, I would like to get more information.";
  const displayMessage = state?.whatsapp?.message?.[language] || state?.whatsapp?.message?.en || defaultMessage;
